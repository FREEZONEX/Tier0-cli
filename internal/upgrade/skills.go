...
func FindSkillsDir(binaryPath string) string {
	// Prefer the canonical independently updateable location. Older release
	// archives placed skill/ beside the binary; keep that only as a fallback.
...
func GetSkillsVersion(skillsDir string) string {
	...
	if meta, err := ReadSkillsMetadata(skillsDir); err == nil && meta.Version != "" {
		return meta.Version
	}
...
func ListSkills(skillsDir string) (*SkillsListResult, error) {
	...
	rootSkillFile := filepath.Join(skillsDir, "SKILL.md")
	if validSkillPackage(skillsDir) {
		name := readSkillFrontmatterField(rootSkillFile, "name")
		if name == "" {
			name = "tier0"
		}
		skills = append(skills, SkillInfo{
			Name:        name,
			Version:     version,
			Description: readSkillDescription(rootSkillFile),
		})
	}
...
func readSkillDescription(skillFile string) string {
	desc := readSkillFrontmatterField(skillFile, "description")
	if len(desc) > 80 {
		desc = desc[:77] + "..."
	}
	return desc
...
func readSkillFrontmatterField(skillFile, field string) string {
...
func skillRepoArchiveURL(commit string) string {
	return fmt.Sprintf("https://github.com/FREEZONEX/Tier0-skill/archive/%s.zip", commit)
}
...
func UpdateSkills(skillsDir string, dryRun bool) (*SkillsUpdateResult, error) {
	...
	if result.UpToDate {
		if !dryRun {
			...
			if metadata.Source != SkillSourceRemote {
				metadata.Version = result.LatestVersion
				metadata.Source = SkillSourceRemote
				metadata.Commit = result.LatestVersion
				metadata.EmbeddedCLIVersion = ""
	...
	archivePath := filepath.Join(tmpDir, "tier0-skill.zip")
	if err := downloadFile(skillRepoArchiveURL(result.LatestVersion), archivePath); err != nil {
	...
	// Record remote provenance so a future CLI upgrade preserves this
	// independently updated Skill rather than replacing it with its baseline.
	metaBytes, _ := json.MarshalIndent(SkillsMetadata{
		Version:   result.LatestVersion,
		Source:    SkillSourceRemote,
		Commit:    result.LatestVersion,
		UpdatedAt: time.Now().UTC().Format(time.RFC3339),
	}, "", "  ")
	parent := filepath.Dir(skillsDir)
	if err := os.MkdirAll(parent, 0o755); err != nil {
		result.ErrorMessage = fmt.Sprintf("failed to create Skill parent directory: %v", err)
		return result, err
	}
	stagingDir, err := os.MkdirTemp(parent, ".tier0-skills-remote-*")
	if err != nil {
		result.ErrorMessage = fmt.Sprintf("failed to create Skill staging directory: %v", err)
		return result, err
	}
	defer os.RemoveAll(stagingDir)
	if err := copyRuntimeSkillDir(srcSkillsDir, stagingDir); err != nil {
		result.ErrorMessage = fmt.Sprintf("failed to stage new Skills: %v", err)
		return result, err
	}
	if err := os.WriteFile(filepath.Join(stagingDir, "_meta.json"), append(metaBytes, '\n'), 0o644); err != nil {
		result.ErrorMessage = fmt.Sprintf("failed to write Skill metadata: %v", err)
		return result, err
	}
	if err := activatePreparedSkill(stagingDir, skillsDir); err != nil {
		result.ErrorMessage = fmt.Sprintf("failed to install new Skills; previous version preserved: %v", err)
		return result, err
	}
...
func copyRuntimeSkillDir(src, dst string) error {
	return filepath.Walk(src, func(path string, info os.FileInfo, err error) error {
		...
		if runtimeSkillPathExcluded(relSlash) {
			if info.IsDir() {
		...
		defer srcFile.Close()
		dstFile, err := os.OpenFile(target, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o644)
		if err != nil {
			return err
		}
...
func runtimeSkillPathExcluded(rel string) bool {
	if rel == "." {
		return false
	}
	...
	if !strings.Contains(rel, "/") {
		switch rel {
		case "README.md", "CHANGELOG.md", "_commit_msg.txt", "install-openclaw.sh", ".gitignore":
			return true
		}
	}
	return rel == "flow/references/protocal" || strings.HasPrefix(rel, "flow/references/protocal/")
}
...
func SkillsLastUpdated(skillsDir string) string {
	...
	if meta, err := ReadSkillsMetadata(skillsDir); err == nil {
		if updatedAt := meta.LastUpdated(); updatedAt != "" {
			return updatedAt
		}
	}
